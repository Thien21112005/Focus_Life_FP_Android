...
public class RunningTrackerService extends Service implements SensorEventListener {
    ...
    private synchronized void persistRunToLocalAndFirebase() {
        ...
        final List<Map<String, Object>> routeData = new ArrayList<>();
        for (Location location : routePoints) {
            Map<String, Object> point = new HashMap<>();
            point.put("lat", location.getLatitude());
            point.put("lng", location.getLongitude());
            point.put("time", location.getTime());
        ...
        new AppExecutors().diskIO().execute(() -> {
            ...
            try {
                ...
                data.put("runningSessionId", runId);
                data.put("updatedAt", now);

                Map<String, Object> sessionData = new HashMap<>(data);
                sessionData.put("routePoints", routeData);
                sessionData.put("startedAt", now - durationMillis);
                sessionData.put("endedAt", now);
                sessionData.put("status", "saved");

                com.google.firebase.firestore.DocumentReference userRef = firestore.collection("users").document(user.getUid());
                com.google.firebase.firestore.WriteBatch batch = firestore.batch();
                batch.set(userRef.collection("step_records").document(runId), data);
                batch.set(userRef.collection("running_sessions").document(runId), sessionData);
                batch.commit()
